...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function displayOrder(res) {
    ...
    res.cars.forEach((item, idx) => {
      ...
      else if (item.insurance === "Additional Driver Insurance") extraCost += 25;
      ...
      orderDetailsDiv.innerHTML += `
        <strong>Car ${idx + 1}:</strong> ${car.brand} ${car.model} – $${parseFloat(car.dailyPrice).toFixed(2)} / day<br>
    ...
    else if (userPoints >= 1000) rate = 0.10;
    ...
    if (rate > 0) {
      ...
      loyaltyMsg.textContent = `Loyalty discount applied: ${rate * 100}% off (–$${discount.toFixed(2)})`;
    } else {
  ...
  applyCouponBtn.addEventListener("click", () => {
    ...
    fetch(`http://localhost:5000/api/coupons/${code}`)
      ...
      .catch(() => { couponMsg.textContent = "Error applying coupon."; });
  });
        ...
        .then(data => {
          if (data?.creditCard) {
            ...
            ["cardHolderName","cardNumber","expiryMonth","expiryYear","cvv"]
              .forEach(id => document.getElementById(id).value = storedCard[id] || "");
          }
        })
        .catch(console.error);
      }
  ...
  completeBtn.addEventListener("click", () => {
    ...
    fetch("http://localhost:5000/api/reservations/updateLineItems", {
      ...
      .then(updatedReservation =>
        ...
        }).then(res => res.ok ? updatedReservation : res.json().then(err => Promise.reject(err)))
      )
      ...
      .then(() => {
        if (appliedCoupon) {
          return fetch(`http://localhost:5000/api/coupons/use/${appliedCoupon}`, {
            method: "PUT"
          });
        }
      })
